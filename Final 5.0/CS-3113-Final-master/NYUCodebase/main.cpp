	...
	while (SDL_PollEvent(&event)) {
		...
		if (event.type == SDL_KEYDOWN) {
			if (event.key.keysym.sym == SDLK_SPACE) {
				Entity* p1Bullet = new Entity(bulletSprite, player1->x + player1->sprite->width/2, player1->y, 0.5, 0.3, 7, 0, 1, 0, 1);
			...
			if (event.key.keysym.sym == SDLK_s) {
				Entity* p2Bullet = new Entity(bulletSprite, player2->x + player2->sprite->width/2, player2->y, 0.5, 0.3, 7, 0, 1, 0, 2);
...
void level1(string fileName){
	...
	if (levelInit){
		...
		player1 = new Entity(p1sheet, 2.5, -6.0, 0.5, 0.8, 0, 0, 0, 0, 0);
		player1->p1 = true;
